...
mESC_high <- read.table("../STARRseq_enhancers/trapRaw_strongESC_clusteredMotifs.txt")
## Read raw TRAP results of weak mESC Enhancers
mESC_low <- read.table("../STARRseq_enhancers/trapRaw_weakESC_clusteredMotifs.txt")

## Read raw TRAP results of strong RARa Enhancers
RARa_high <- read.table("../STARRseq_enhancers/trapRaw_strongRARa_clusteredMotifs.txt")
## Read raw TRAP results of weak RARa Enhancers
RARa_low <- read.table("../STARRseq_enhancers/trapRaw_weakRARa_clusteredMotifs.txt")


## prepare value matrix for strong mESC enhancers
l_mESC <- nrow(mESC_high)/79  # 257
profiles <- mESC_high[1:79,4] 
values_mESC_high <- c()
for (k in 1:l_mESC){
  col <- mESC_high[(1+(k-1)*79):(k*79),5]
...
l_RARa <- nrow(RARa_high)/79  # 1141
values_RARa_high <- c()
for (k in 1:l_RARa){
  col <- RARa_high[(1+(k-1)*79):(k*79),5]
...
write.table(cbind(pvalues_mESC_high,padj_mESC_high),"../RESULTS_trap/trap_strongESC_clusteredMotifs.txt", quote = F)
...
write.table(cbind(pvalues_mESC_low,padj_mESC_low),"../RESULTS_trap/trap_weakESC_clusteredMotifs.txt", quote = F)
...
write.table(cbind(pvalues_RARa_high,padj_RARa_high),"../RESULTS_trap/trap_strongRARa_clusteredMotifs.txt", quote = F)
...
write.table(cbind(pvalues_RARa_low,padj_RARa_low),"../RESULTS_trap/trap_weakRARa_clusteredMotifs.txt", quote = F)
